...
export class CreateZoneSlotConfigDto {
  ...
  @ApiProperty({ example: 1.63, required: false, nullable: true, description: 'Length in mm' })
  @IsOptional()
  @IsNumber()
  dim_l_mm: number | null;

  @ApiProperty({ example: 1.63, required: false, nullable: true, description: 'Width in mm' })
  @IsOptional()
  @IsNumber()
  dim_w_mm: number | null;

  @ApiProperty({ example: true, description: 'Is the slot quantity dynamic by ring size?' })
  @IsBoolean()
  is_dynamic_by_size: boolean;

  @ApiProperty({ type: [SizeWtMatrixEntryInputDto], required: false, description: 'Size weight matrix entries' })
  @IsOptional()
  @IsArray()
  ...
  @ApiProperty({ example: 1, required: false, nullable: true, description: 'Fixed quantity when is_dynamic_by_size is false' })
  @IsOptional()
  @IsNumber()
